...
typedef struct {
  ...
  char text[MSG_LEN];  // The message itself
} msg_t;
...
int send_msg (int key, int type, char *message)
{
  ...
  printf("Message send successfully to queue with key %d (id: %d)\n",
      key, msg_id);
...
int rcv_msg (int key, int type)
{
  ...
  printf("Message received successfully from queue with key %d (id: %d)\n" \
      "MESSAGE: %s\n", key, msg_id, msg.text);
