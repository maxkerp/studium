...
//## Gruppe: 5                                                                  ##
//## Erstellungsdatum: 06.06.2014                                               ##
//## Funktionsumfang A: Threads u. Mutex                                        ##
//## Aufgabenblatt: Uebung 9                                                    ##
//## Info: Doppelseitig bedruckt                                                ##
//################################################################################

/* Task 9
 * The task was to create a global float array for account_balances. Their value is
 * the account amount. Then we should create two threads, which are writing from
 * account 1 to 2 and from account 2 to 3. For the transfer we should create a function
 * called process_transfers. While transfering data we have to secure writing to 
 * the account balance array with a mutex.
 */
...
int process_transfer (int from_acct, int to_acct, float amount) {
  ...
  printf("Starting transfer, locking for this transfer:\n");
  ...
  //take amount from the from account
  account_balances[from_acct] -= amount;
  //send it to the to account
  account_balances[to_acct] += amount;

  //now unlock the accounts for other threads
  account_mutex[from_acct] = 0;
  account_mutex[to_acct] =  0;

  printf("Finished transfer. Accounts unlocked.\n\n");
  return 1;
}
