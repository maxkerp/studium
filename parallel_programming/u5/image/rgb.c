...
int GraytoRGB(int height, int width, gray_t gray_image[height][width],
	      myrgb_t rgb_image[height][width])
{
  /* convert pixels */
...
int RGBDisplay(int height, int width, myrgb_t image[height][width])
{
  char c;
  int window_number;
  ...
  for(int i=0; i<height; i++)
      for(int j=0; j<width; j++)
	{
	  myrgb_t rgb_color = image[height-i-1][j];
