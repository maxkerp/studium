		      ...
		      gray_t new_image[height][width])    // O: output image
{
  ...
  t0 = gettime();
#pragma omp target map(to:image[0:height][0:width]) map(tofrom:new_image[0:height][0:width])
{
...
#pragma omp target map(tofrom:tmp_image1[0:height][0:width], tmp_image2[0:height][0:width], gray_image[0:height][0:width]) map(from:strength[0:height][0:width], direction[0:height][0:width])
{
				...
				#pragma omp parallel
				{
					...
					#pragma omp for private (j, dx, dy, tmp)
					for(i=0; i<height; i++)
						for(j=0; j<width; j++) {
